...
unique_desc = data['text'].unique().tolist()
...
def get_val(data_path='data/', val=0.8, seed=0):
    ...
    # read caption file
    # data = pd.read_csv(os.path.join(data_path, 'captions.csv')) 
    # data = data[data['subtype'].isin(SUBTYPES)]
    # unique_desc = data['text'].unique().tolist()
    # data = data.values.tolist()

    ids = {}
    id_i = [[] for _ in range(len(unique_desc))]
    # text = {}
    for i, s, t in data:
        ids[i] = unique_desc.index(t) 
        id_i[unique_desc.index(t)].append(i)  
        # if s in SUBTYPE_CLS:
            # ids[i] = SUBTYPE_CLS[s]
            # id_i[SUBTYPE_CLS[s]].append(i)
            # text[i] = t
    os.makedirs(f'data/gastric_cls3_ann_desc', exist_ok=True)
    TRAIN_PATH = f'data/gastric_cls3_ann_desc/train_all_{round(1 - val, 2)}.txt'
    VAL_PATH = f'data/gastric_cls3_ann_desc/val_{val}.txt'
    ...
    file_i = [[] for _ in range(len(unique_desc))]
    ...
    for file in file_list:
        ...
        for j, id_j in enumerate(id_i):
            if i in id_j:
                # file_i[j].append(file + f' {j}'+f' {text[i]}')
                file_i[j].append(file + f' {j}')
                m[j] += 1
                break
        else:
            if i in ids:
                # train_files.append(file + f' {ids[i]}'+f' {text[i]}')
...
def get_patient(train_file='data/gastric_cls3_ann_desc/train_all_0.2.txt',
                num_patients=1,
                seed=0,
                sort=True):
    ...
    file_i = [[] for i in range(len(unique_desc))]
    # text_i = [[] for i in range(3)]
    for file in train_files:
        cls = int(file.split(' ')[-1])
        file_i[cls].append(file)#.split(' ')[0:2])
        # text_i[cls].append(file.split(' ')[2:])
    ...
    train_file_i = [[] for i in range(len(unique_desc))]
    for i in id_i:
