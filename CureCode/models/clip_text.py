...
@BACKBONES.register_module()
class PromptedCLIPImageBackboneWithText(CLIPImageBackbone):
    ...
    def __init__(self,
        ...
        self.text_model = AutoModel.from_pretrained('michiyasunaga/BioLinkBERT-large')
        self.text_tokenizer = AutoTokenizer.from_pretrained('michiyasunaga/BioLinkBERT-large')
    ...
    def forward(self, x):
        x = x.type(self.dtype)
        # print(x)
#        text_description = text_description.type(self.dtype)
        ...
        # with torch.no_grad():
        #     text_description_features = self.text_model(**self.text_tokenizer(text_description, return_tensors='pt'))['last_hidden_state']

        # prompt: [layer, length, batch, embed_dim]
        if self.prompt_pos == 'prepend':
            x = torch.cat([x[:1, :, :], prompt[0, :, :, :], x[1:, :, :]], dim=0)

        # x = torch.cat([x, text_description_features], dim=0)
